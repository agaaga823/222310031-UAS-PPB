...
export default function MainPage({ navigation }) {
  ...
  const [filteredData, setFilteredData] = useState(sampah);

  useEffect(() => {
    if (searchQuery === '') {
      setFilteredData(sampah);
    } else {
      const filtered = sampah.map(section => {
        ...
        renderSectionHeader={({ section: { category, data } }) => (
          <View style={styles.section}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.itemsRow}>
              {data.map((item) => (
                <TouchableOpacity key={item.id} onPress={() => navigation.navigate('DetailPage', item)}>
